...
<div class="bg-white rounded-xl shadow p-4">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
        <div><span class="text-gray-400">Mode:</span> <span class="font-medium capitalize">{{ $purchase->payment_mode }}</span></div>
        <div><span class="text-gray-400">Discount:</span> <span class="font-medium">Rs. {{ number_format($purchase->discount, 2) }}</span></div>
        <div><span class="text-gray-400">Tax:</span> <span class="font-medium">Rs. {{ number_format($purchase->tax, 2) }}</span></div>
        @if($purchase->memo)
        <div class="col-span-2"><span class="text-gray-400">Memo:</span> <span class="font-medium">{{ $purchase->memo }}</span></div>
        @endif
    </div>
</div>
