                    ...
                    @forelse($stocks->sortBy('product.name') as $stock)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-xs text-gray-500">
                                {{ $stock->power->subclass->lensClass->category->name ?? '—' }}
                            </td>
                            <td class="px-4 py-3 text-xs text-gray-500">
                                {{ $stock->power->subclass->lensClass->name ?? '—' }}
                            </td>
                            <td class="px-4 py-3 text-xs text-gray-600">
                                {{ $stock->power->subclass->name ?? '—' }}
                            </td>
                            <td class="px-4 py-3">{{ $stock->power ? $stock->power->getLabel() : '—' }}</td>
                            <td class="px-4 py-3 text-center">
                                @if($stock->quantity < 5)
                                    <span
                                        class="bg-red-100 text-red-700 px-2 py-1 rounded text-xs font-bold">{{ $stock->quantity }}</span>
                                @else
                                    <span
                                        class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold">{{ $stock->quantity }}</span>
                                @endif
                            </td>
                        </tr>
                    @empty
            ...
            @forelse($stocks->sortBy('product.name') as $stock)
                <div class="bg-white rounded-xl shadow p-4 flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-sm text-gray-600">{{ $stock->power ? $stock->power->getLabel() : '—' }}</p>
        ...
        <div class="hidden sm:block bg-white rounded-xl shadow overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $products->count() + 1 }}" class="px-4 py-8 text-center text-gray-400">No stock found
                            </td>
                        </tr>
                    @endforelse
                </tbody>
